...
if page == "Home":
    ...
    departure_cities = sorted([row[0] for row in cursor.fetchall()])  # Sort departure cities alphabetically

    cursor.execute("SELECT DISTINCT Destination FROM bus_detail WHERE Destination IS NOT NULL")
    destinations = sorted([row[0] for row in cursor.fetchall()])  # Sort destinations alphabetically
